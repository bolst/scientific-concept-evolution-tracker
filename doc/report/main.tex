...
For ``Entropy'', the system identifies a two concepts between the physical and information sciences.
...
For instance, a sudden increase in the variance of a concept's embedding cluster might indicate a split of consensus or the birth of a new application domain.
...
However, limiting the analysis to the top 1000 papers means we might miss subtle, long-running evolutionary trends that appear in lower-ranked documents.
...
By integrating a hybrid vector search engine (Milvus) with a relational metadata store (PostgreSQL), we achieved a system that is both powerful in semantics and lexically precise.
Our methodology of combining dynamic K-Means clustering with temporal segmentation allows users to inherently discover distinct sub-concepts and their active eras with convenience.
...
Case studies on ``Transformer'', ``Entropy'', ``Token'' confirmed the system's ability to correctly disambiguate similar terms and identify historical turning points.
